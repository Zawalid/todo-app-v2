...
export default function useDeleteMultiple({
  ...
  return {
    ...
    Modal: (
      <div
        className={`fixed left-1/2 z-[999999] flex w-[90%] -translate-x-1/2 items-center justify-between rounded-lg border bg-background-primary px-5 py-3 shadow-lg transition-[bottom] duration-500 sm:w-[500px]  ${
          isDeleteMultipleModalOpen ? 'bottom-11' : '-bottom-[100px]'
        }`}
      >
        <h2 className='text-sm font-semibold text-text-secondary '>
          <span className='mr-2 rounded-md bg-text-secondary px-2 py-1 text-white  '>
            {selectedItemsNumber}
          </span>
        ...
        <div className='flex items-center  gap-3 '>
          ...
          <button
            className='rounded-lg bg-red-500 px-3 py-2 text-xs font-semibold text-background-secondary transition-colors duration-300 hover:bg-red-600 sm:px-4'
            onClick={onConfirm}
          >
            Delete
          </button>
